...
export default function MaintenanceDashboard({ societyId }) {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/maintenance/all/${activeSocietyId}`, {
        headers: {
  ...
  const handleVerify = async (id, status) => {
    try {
      setIsLoading(true);
      await axios.put(`${import.meta.env.VITE_API_URL}/api/maintenance/verify/${id}`, 
        { status }, 
        {
          headers: {
